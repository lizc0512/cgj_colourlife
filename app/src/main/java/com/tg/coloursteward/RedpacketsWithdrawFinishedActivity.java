...
public class RedpacketsWithdrawFinishedActivity extends BaseActivity {
	...
	private void carryOrderCreate() {
		...
		if ("colleague".equals(transferTo) || "czy".equals(transferTo)) {
			// 给同事发红包 或 转账到彩之云
			receiverId = getIntent().getStringExtra(Contants.PARAMETER.USERID);
			transferNote = getIntent().getStringExtra(Contants.PARAMETER.TRANSFERNOTE);
			type = getIntent().getStringExtra("type");
			params.put("receiver_id", receiverId);
			params.put("type", type);
			params.put("amount", withdrawAmount);
