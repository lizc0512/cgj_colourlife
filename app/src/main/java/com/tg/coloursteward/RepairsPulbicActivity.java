...
import com.tg.coloursteward.RepairsPulbicActivity;
...
import com.tg.coloursteward.view.ImageViewGroup;
...
public class RepairsPulbicActivity extends BaseActivity {
	...
	protected boolean handClickEvent(View v) {
		...
		case R.id.right_layout:// 提交
			...
			if(list == null || list.size() == 0){
				...
				HttpTools.postImages(Contants.URl.URL_ICETEST,mHand, list);
			}
			break;
		}
	...
	private void submit() {
		...
		HttpTools.httpPost(Contants.URl.URL_ICETEST,"/complainRepairs" ,config, params);
	}
		...
		if (typeDialog == null) {
			...
			typeDialog.setNetworkListener(new NetworkRequestListener() {
				...
				public void onRequest(MessageHandler msgHand) {
					...
					params.put("state", 2);
					params.put("type", 1);
					HttpTools.httpGet(Contants.URl.URL_ICETEST,"/category/select", 
							config, params);
				}
