...
public class PieView extends View {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		TypedArray typeArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.PieView, 0, 0 );
		// 圆环大小默认为屏幕的六分之一宽度
		mRingRadius = typeArray.getDimension(R.styleable.PieView_radiusPie, mScreenWdith / 6);
		//默认为半径的三分之一
		mRingWidth = typeArray.getDimension(R.styleable.PieView_ringWidth, mRingRadius / 3);
		//默认线长为圆环宽度
		mLineLength = typeArray.getDimension(R.styleable.PieView_lineLength, mRingRadius / 3 );
		//文字大小
		mTextSize = typeArray.getDimension(R.styleable.PieView_textSize, sp2px(context, 10));
		//方块边长默认为文字大小
		mRectWdith = typeArray.getDimension(R.styleable.PieView_squareSide, mTextSize);
