 ...
 * Description:
 */
public class CacheEmployeeHelper {

    private static CacheEmployeeHelper instance;


    public static CacheEmployeeHelper instance() {
        if (instance == null) {
            instance = new CacheEmployeeHelper();
        }
        return instance;
    }

    private CacheEmployeeHelper() {
     ...
     * @param bean
     */
    public void insertOrUpdate(Context context, EmployeeBean bean) {
        EmployeeBeanDao dao = GreenDBIMManager.instance(context).getCacheEmployeeDao();

        List<EmployeeBean> list = dao.queryBuilder()
                .where(EmployeeBeanDao.Properties.Uid.eq(bean.getUid())).list();
    ...
    public List<CacheMsgBean> toQueryCacheMsgListAndSetRead(Context context, String dstUuid, boolean setRead) {
        ...
        if (setRead) {
            ...
            if (unReadList.size() > 0) {
                CacheEmployeeHelper.instance().updateList(context, unReadList);
            }
        }
    ...
    public List<CacheMsgBean> getCacheMsgBeanListFromStartIndex(Context context, long startIndex,
        ...
        List<CacheMsgBean> list =
                CacheEmployeeHelper.instance().toQueryOrAscById(context, startIndex, selfUuid, dstUuid);
