...
public class AbDBDaoImpl<T> extends AbBasicDBDao implements AbDBDao<T> {
    ...
    private static final String TAG = AbDBDaoImpl.class.getSimpleName();
    /**
     * The db helper.
     */
    ...
    public List<T> queryList(String[] columns, String selection,
        ...
        try {
            ...
            Log.e(TAG, "[queryList] from DB Exception");
            e.printStackTrace();
        } finally {
            closeCursor(cursor);
            lock.unlock();
    ...
    public long[] insertList(List<T> entityList, boolean flag) {
        ...
        try {
            ...
            Log.d(TAG, "[insertList] into DB Exception.");
            e.printStackTrace();
        } finally {
            lock.unlock();
        }
     ...
     */
    @Override
    public int update(T entity, String column) {
        int rows = 0;
        try {
            lock.lock();
            checkDBOpened();
            ...
            String colName = cv.get(column).toString();
            String[] whereValue = {colName};
            rows = db.update(this.tableName, cv, column + "=?", whereValue);
    ...
    public int update(T entity) {
        ...
        try {
            ...
            String where = this.idColumn + " = ?";
            Log.e("mm", "icColumn = " + this.idColumn);
    ...
    private String setContentValues(T entity, ContentValues cv, int type,
        ...
        // 加载所有字段
        List<Field> allFields = AbTableHelper.joinFields(entity.getClass().getDeclaredFields(),
                entity.getClass().getSuperclass().getFields());
        for (Field field : allFields) {
            if (!field.isAnnotationPresent(Column.class)) {
                continue;
            }
     ...
     * 描述：查询为map列表.
     *
     * @param selection     the sql
     * @param selectionArgs the selection args
     * @return the list
     * @see AbDBDao#queryMapList(String, String[])
     */
    @Override
    public List<Map<String, String>> queryMapList(String selection, String[] selectionArgs) {
        Cursor cursor = null;
        List<Map<String, String>> retList = new ArrayList<Map<String, String>>();
        try {
            lock.lock();
            checkDBOpened();
            Log.d(TAG, "[queryMapList]: " + getLogSql(selection, selectionArgs));
    ...
    public int queryCount(String selection, String[] selectionArgs) {
        ...
        try {
            lock.lock();
            checkDBOpened();
            Log.d(TAG, "[queryCount]: " + getLogSql(selection, selectionArgs));
     ...
     */
    @Override
    public void execSql(String selection, Object[] selectionArgs) {
        try {
            lock.lock();
            checkDBOpened();
            Log.d(TAG, "[execSql]: " + getLogSql(selection, selectionArgs));
