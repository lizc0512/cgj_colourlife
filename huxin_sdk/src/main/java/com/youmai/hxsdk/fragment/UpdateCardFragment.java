...
public class UpdateCardFragment extends BaseFragment implements View.OnClickListener {
    public final static String TAG = UpdateCardFragment.class.getSimpleName();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.hx_fragment_update_card, container, false);
    }
    ...
    private void updateCardInfo(YouMaiBizCard.BizCard.Builder cardBulider) {
        int userId = HuxinSdkManager.instance().getUserId();
        String phone = HuxinSdkManager.instance().getPhoneNum();
            ...
            public void OnRec(PduBase pduBase) {
                try {
                    ...
                    if (ack.getErrerNo() == YouMaiBasic.ERRNO_CODE.ERRNO_CODE_OK) {
                        Toast.makeText(mAct, getString(R.string.hx_toast_48), Toast.LENGTH_SHORT).show();
                    }
    ...
    private YouMaiBizCard.BizCard.Builder getCardBulider() {
        ...
        cardBulider.setEmail(email);
        cardBulider.setJob(job);
        cardBulider.setTelephone(tel);
        cardBulider.setAddress(addr);
        cardBulider.setWebsite(web);
        cardBulider.setCompanyName(com);
        cardBulider.setNickname(nickname);
        cardBulider.setGender(gender);
        cardBulider.setArea(area);
        cardBulider.setSummary(summary);
        try {
    ...
    public boolean checkInput() {
        ...
        if (!StringUtils.isEmpty(email) && !AppUtils.isEmail(email)) {      //邮箱格式不正确
            Toast.makeText(getActivity().getApplicationContext(), getString(R.string.hx_toast_46),
                    Toast.LENGTH_SHORT).show();
            return false;
        }
        if (!StringUtils.isEmpty(tel) && !AppUtils.isTelephone(tel)) {        //座机格式不正确

            Toast.makeText(getActivity().getApplicationContext(), getString(R.string.hx_toast_47),
                    Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
        ...
        ReceiveListener callback = new ReceiveListener() {
            ...
            public void OnRec(PduBase pduBase) {
                try {
                    YouMaiBizCard.BizCard_Update_Ack ack = YouMaiBizCard.BizCard_Update_Ack.parseFrom(pduBase.body);
                    if (ack.getErrerNo() == YouMaiBasic.ERRNO_CODE.ERRNO_CODE_OK) {
                        Toast.makeText(mAct, getString(R.string.hx_toast_49), Toast.LENGTH_SHORT).show();
                    }
